...
Rate limiting реализован как простой in-memory limiter с отдельным sliding window для каждого клиента.

Устаревшие записи клиентов периодически удаляются из памяти, поэтому адреса клиентов, которые больше не обращаются к сервису, не накапливаются в течение всего времени жизни процесса.

В текущей конфигурации Docker приложение запускается одним Uvicorn worker. Состояние rate limiter хранится только в памяти процесса и не разделяется между несколькими workers или несколькими экземплярами приложения.

Если сервис будет запущен с несколькими workers или репликами, каждый процесс будет иметь собственный независимый счётчик. Для строгого общего лимита в такой конфигурации потребуется shared storage, например Redis, либо rate limiting на уровне reverse proxy/API gateway.
...
Если в будущем сервис будет размещён за nginx, Traefik или другим reverse proxy, непосредственным клиентом FastAPI станет сам proxy. В таком deployment необходимо отдельно настроить trusted proxy headers, чтобы получать исходный IP пользователя.

Приложение намеренно не доверяет произвольному `X-Forwarded-For` самостоятельно: при прямом доступе клиент может подделать этот HTTP-заголовок и тем самым обходить IP-based rate limit. Доверять forwarded headers следует только от явно доверенного reverse proxy.
